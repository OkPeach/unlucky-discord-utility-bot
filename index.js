...
// Log voice channel mutes and deafens
client.on('voiceStateUpdate', async (oldState, newState) => {
  const member = newState.member;

  // Check for voice channel mute changes
  if (oldState.mute !== newState.mute) {
    const embed = new EmbedBuilder()
      .setColor('#' + process.env.EMBEDCOLOR)
      .setTitle('Voice Channel Mute Updated')
      .addFields(
        { name: 'Member', value: `${member.user.tag} (${member.user.id})`, inline: false },
        { name: 'Muted', value: newState.mute ? 'Yes' : 'No', inline: false },
        { name: 'Channel', value: newState.channel ? `<#${newState.channel.id}>` : 'Not in a voice channel', inline: false }
      )
      .setFooter({ text: 'Unlucky bot | Made by unlucky.life' })
      .setTimestamp();
  ...
  // Check for voice channel deafen changes
  if (oldState.deaf !== newState.deaf) {
    const embed = new EmbedBuilder()
      .setColor('#' + process.env.EMBEDCOLOR)
      .setTitle('Voice Channel Deafen Updated')
      .addFields(
        { name: 'Member', value: `${member.user.tag} (${member.user.id})`, inline: false },
        { name: 'Deafened', value: newState.deaf ? 'Yes' : 'No', inline: false },
        { name: 'Channel', value: newState.channel ? `<#${newState.channel.id}>` : 'Not in a voice channel', inline: false }
      )
      .setFooter({ text: 'Unlucky bot | Made by unlucky.life' })
      .setTimestamp();
